...
arakoon_rc arakoon_cluster_connect_master(ArakoonCluster * const cluster,
    const ArakoonClientCallOptions * const options) {
        ...
        while(node != NULL) {
                ...
                if(ARAKOON_RC_IS_SUCCESS(rc)) {
                        _arakoon_log_debug("Connected to node %s", node->name);
                        break;
                }
        ...
        if(strcmp(node->name, master) == 0) {
                ...
                _arakoon_log_info("Found master node %s", cluster->master->name);
        ...
        _arakoon_log_debug("Connecting to master node %s", node->name);
        ...
        _arakoon_log_debug("Found master node %s", cluster->master->name);
